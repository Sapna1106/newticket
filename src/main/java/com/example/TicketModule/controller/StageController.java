...
import com.example.TicketModule.dto.rule.CustomResponseEntity;
import com.example.TicketModule.dto.rule.StageDto;
import com.example.TicketModule.entity.Stage;
import com.example.TicketModule.exception.StageNotFoundException;
import com.example.TicketModule.service.StageService;
...
public class StageController {
    ...
    public ResponseEntity<?> addNewStage(@PathVariable long projectId, @RequestBody Stage addStage) {
        try {
            ...
            stageDto.setStageId(addStage.getId());
    ...
    public ResponseEntity<?> updateStageById(@PathVariable long projectId, @RequestBody Stage updateStage) {
        try {
            ...
            logger.error("Stage with ID {} Not Found", updateStage.getId());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                    new CustomResponseEntity("Stage with ID " + updateStage.getId() + " Not Found", HttpStatus.NOT_FOUND.value(), null)
            );
        } catch (Exception ex) {
